...
void openLastSaved() {
    free(head);
    ifstream file("atividade5.1.dat");
    Point point;
...
void save() {
    ofstream file("atividade5.1.dat");
    Node *current = head;
    while (current != NULL) {
...
void rotate() {
    ...
    while (current != NULL) {
        float dx = current->point.x - center_x;
        float dy = current->point.y - center_y;
        current->point.x = center_x + (dx * cos(0.1) - dy * sin(0.1));
        current->point.y = center_y + (dx * sin(0.1) + dy * cos(0.1));
        current = current->next;
    }
